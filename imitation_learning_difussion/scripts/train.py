...
def run_epoch(model, scheduler, loader, optimizer, device, include_force: bool) -> float:
...
def main() -> None:
    ...
    for epoch in range(1, int(config["training"]["epochs"]) + 1):
        train_loss = run_epoch(model, scheduler, train_loader, optimizer, device, include_force)
        val_loss = run_epoch(model, scheduler, val_loader, None, device, include_force)
