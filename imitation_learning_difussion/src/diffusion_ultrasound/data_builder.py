...
def split_sessions(session_dirs: list[Path], config: dict[str, Any]) -> dict[str, list[Path]]:
    ...
    prefix = str(config["input"].get("session_prefix", "session_"))
    return {
        "train": sorted(shuffled[:train_count], key=lambda path: session_sort_key(path, prefix)),
        "val": sorted(shuffled[train_count : train_count + val_count], key=lambda path: session_sort_key(path, prefix)),
        "test": sorted(shuffled[train_count + val_count :], key=lambda path: session_sort_key(path, prefix)),
    }


def build_session_samples(session_dir: Path, config: dict[str, Any], *, include_force: bool) -> tuple[list[dict[str, Any]], dict[str, int]]:
...
def copy_sample_images(samples: list[dict[str, Any]], image_root: Path, *, desc: str) -> None:
    for sample in tqdm(samples, desc=desc, unit="img", leave=False):
        ...
        sample["image"] = str(Path("images") / session_id / target_name)
...
def build_dataset(config_path: str | Path) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "splits": {name: [path.name for path in paths] for name, paths in split_map.items()},
        "versions": {},
    }
    ...
    for version in tqdm(versions, desc="dataset versions", unit="version"):
        ...
        for split_name, split_sessions_list in tqdm(split_map.items(), desc=f"{version} splits", unit="split", leave=False):
            ...
            write_json(split_root / "samples.json", all_samples)
            write_json(split_root / "split_summary.json", split_summary)
