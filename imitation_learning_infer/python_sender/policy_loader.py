...
ACT_SRC = RESEARCH_ROOT / "imitation_learning_ACT" / "src"
DIFFUSION_SRC = RESEARCH_ROOT / "imitation_learning_difussion" / "src"
...
class PolicyRunner:
    ...
    def _load_model(self, policy_cfg: dict[str, Any]) -> torch.nn.Module:
        ...
        if self.policy_type == "act":
            _add_import_path(ACT_SRC)
            from act_ultrasound.model import ACTPolicy
            ...
            _add_import_path(DIFFUSION_SRC)
            from diffusion_ultrasound.model import ResNet18ConditionedDenoiser
