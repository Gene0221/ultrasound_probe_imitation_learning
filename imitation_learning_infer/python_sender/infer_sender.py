...
from config import INFER_ROOT, load_config
from force6d_monitor import ForceSafetyMonitor
from policy_loader import PolicyRunner
from ultrasound_source import ImageFolderUltrasoundSource, LiveCameraUltrasoundSource


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run realtime ultrasound policy inference and stream action chunks.")
    parser.add_argument("--config", default=str(INFER_ROOT / "config" / "default.yaml"))
...
def build_source(args: argparse.Namespace, config: dict[str, Any]):
    if args.image_dir is None:
        ...
        if source_type == "image_folder":
            ...
            return ImageFolderUltrasoundSource(Path(str(image_dir)))
