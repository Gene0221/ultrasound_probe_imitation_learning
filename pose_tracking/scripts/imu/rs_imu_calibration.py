...
RAW_DATA_DIR = PROJECT_ROOT / "data" / "imu" / "raw"
    ...
    try:
        ...
        if accel_file:
            ...
            if header:
                ...
                accel_file = str(RAW_DATA_DIR / ('accel_%s.txt' % header))
                gyro_file = str(RAW_DATA_DIR / ('gyro_%s.txt' % header))
                print('Writing files:\n%s\n%s' % (accel_file, gyro_file))
                np.savetxt(accel_file, on_mm, delimiter=',', fmt='%s')
        ...
        directory = os.path.dirname(accel_file) if accel_file else str(CALIBRATION_DIR)
        ...
        with open(os.path.join(directory,"calibration.bin"), 'wb') as outfile:
            outfile.write(imu_calib_table.astype('f').tobytes())
