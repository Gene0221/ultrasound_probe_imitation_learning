...
class TemporalPolicyNet(nn.Module):
    def __init__(
        ...
        gru_input_dim = self.image_encoder.output_dim + pose_feature_dim + (force_feature_dim if use_force else 0)
        effective_gru_dropout = gru_dropout if gru_num_layers > 1 else 0.0
        self.gru = nn.GRU(
